...
public class BathLoader {
    public static void init() {
        ...
        for (DyeColor dye : DyeColor.values()){
            ...
            BATH.RB().fi(FluidIngredient.of(dyeLiquid, L / 2)).ii(Items.CANDLE).io(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_candle"))).add(dye.getName() + "_candle", 64);
            BATH.RB().fake().ii(Items.SHULKER_BOX).fi(FluidIngredient.of(dyeLiquid, L / 2)).io(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_shulker_box"))).add(dye.getName() + "_shulker_box", 64);
            BATH.RB().fake().ii(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_shulker_box"))).fi(Chlorine.getGas(50)).io(Items.SHULKER_BOX).add("shulker_box_from_" + dye.getName(), 64);

            if (dye != DyeColor.WHITE){
                BATH.RB().fi(FluidIngredient.of(dyeLiquid, L / 2)).ii(Items.WHITE_WOOL).io(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_wool"))).add(dye.getName() + "_wool", 64);
                BATH.RB().fi(Chlorine.getGas(50)).ii(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_wool"))).io(Items.WHITE_WOOL).add("white_wool_from_" + dyeName, 400);
                BATH.RB().fi(Chlorine.getGas(25)).ii(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_carpet"))).io(Items.WHITE_CARPET).add("white_carpet_from_" + dyeName, 400);
                BATH.RB().fi(FluidIngredient.of(dyeLiquid, L / 2)).ii(Items.WHITE_BED).io(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_bed"))).add(dye.getName() + "_bed", 64);
                BATH.RB().fi(Chlorine.getGas(50)).ii(RegistryUtils.getItemFromID(new ResourceLocation(dye.getName() + "_bed"))).io(Items.WHITE_BED).add("white_bed_from_" + dyeName, 400);
            }
        }
        mercurybathing();
        persulfatebathing();

...
public class ExtenderFluidHandler<T extends BlockEntityMachine<T>> extends MachineFluidHandler<T> {

    public ExtenderFluidHandler(T tile) {
        super(tile);
    }

    @Override
    public LazyOptional<IFluidHandler> forSide(Direction side) {
        return LazyOptional.of(() -> new ExtenderSidedWrapper(this, tile.coverHandler.map(c -> c).orElse(null), side));
    }

    LazyOptional<IFluidHandler> getFluidHandler(Direction facing) {
        Direction side = facing == tile.getFacing() ? tile.getOutputFacing()  : tile.getFacing();
        BlockEntity entity = tile.getCachedBlockEntity(side);
        return entity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, side.getOpposite());
    }

    private class ExtenderSidedWrapper extends FluidHandlerSidedWrapper {

        public ExtenderSidedWrapper(IFluidNode fluidHandler, CoverHandler<?> coverHandler, Direction side) {

    ...
    public static void init() {
        ...
        DENSE_PLATE.all().forEach(t -> {
            ...
            if (t.has(GTMaterialTypes.INGOT)) {
                BENDER.RB().ii(GTMaterialTypes.INGOT.getMaterialIngredient(t,9),SELECTOR_TAG_INGREDIENTS.get(9)).io(DENSE_PLATE.get(t,1)).add("plate_dense_" + t.getId(), baseDuration.applyAsLong(t) * 9, 96);
            }
        });
        ...
        GTMaterialTypes.SPRING.all().stream().filter(m -> !m.has(MaterialTags.RUBBERTOOLS)).forEach(spring -> {
            BENDER.RB().ii(LONG_ROD.getMaterialIngredient(spring, 1), SELECTOR_TAG_INGREDIENTS.get(1)).io(SPRING.get(spring)).add(spring.getId() + "_spring", 200, 16);
        });

...
public class ThermalCentrifugeLoader {
    public static void init() {
        GTMaterialTypes.CRUSHED_PURIFIED.all().forEach(m -> {
            ...
            THERMAL_CENTRIFUGE.RB().ii(RecipeIngredient.of(GTMaterialTypes.CRUSHED_PURIFIED.get(m),1)).io(GTMaterialTypes.CRUSHED_REFINED.get(m, 1), GTMaterialTypes.DUST_TINY.get(aOreByProduct2, 1)).add("purified_" + m.getId(),500, 48,0,2);
            THERMAL_CENTRIFUGE.RB().ii(RecipeIngredient.of(GTMaterialTypes.CRUSHED.get(m),1)).io(GTMaterialTypes.CRUSHED_REFINED.get(m, 1), GTMaterialTypes.DUST_TINY.get(aOreByProduct2, 1), stoneDust).add("crushed_" + m.getId(),500, 48,0,2);
        });
    }
}

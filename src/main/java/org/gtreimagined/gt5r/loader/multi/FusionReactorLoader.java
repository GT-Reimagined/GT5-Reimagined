...
public class FusionReactorLoader {
    public static void init(){
        ...
        FUSION.RB().huOutput(8192).fi(Magnesium.getLiquid(L), Silicon.getLiquid(L)).fo(Iron.getLiquid(L)).add("molten_iron", 288, 16, 360000000); //plasma
        FUSION.RB().fi(Hydrogen.getGas(125), Manganese.getLiquid(L / 9)).fo(Iron.getLiquid(L / 9)).add("molten_iron_2", 64, 8192, 120000000);
        FUSION.RB().huOutput(10240).fi(Lithium.getLiquid(L), Aluminium.getLiquid(L)).fo(Sulfur.getLiquid(L)).add("molten_sulfur", 288, 24, 240000000); //plasma
        FUSION.RB().huOutput(16384).fi(Hydrogen.getGas(375), Beryllium.getLiquid(L / 9)).fo(Nitrogen.getGas(175)).add("nitrogen", 16, 32, 180000000); //plasma
